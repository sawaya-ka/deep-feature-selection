...
# ============================================================
# === Additions: Split-aware generators & training pipelines ===
#   (Non-breaking: all original APIs remain unchanged)
# ============================================================

import numpy as _np
from typing import Sequence as _Sequence, Literal as _Literal

# ----- Split-aware design generator -----
Scenario = _Literal["gaussian", "t3", "spiked", "var"]

def _haar_frame(d: int, r: int, rng: _np.random.Generator) -> _np.ndarray:
    Z = rng.normal(size=(d, d))
    Q, R = _np.linalg.qr(Z)
    s = _np.sign(_np.diag(R)); s[s == 0] = 1.0
    Q = Q @ _np.diag(s)
    return Q[:, :r]

def _wishart(df: int, n: int, rng: _np.random.Generator) -> _np.ndarray:
    Y = rng.normal(size=(df, n))
    return Y.T @ Y

def generate_design_with_splits(
    m: int,
    n: int,
    scenario: Scenario,
    seed: int | None = None,
    *,
    r_spike: int = 2,
    var_alpha: _Sequence[float] | None = None,
    var_burnin: int = 200,
):
    """
    Return (X_full, X_A, X_B) where X_A, X_B are the two equal row-halves.
    Spiked/VAR are generated so that each half is itself a valid sample from the same model.
    """
    ...
    if scenario == "gaussian":
        X = rng.normal(scale=1.0 / _np.sqrt(n), size=(m, n))
        return X, X[:m2, :].copy(), X[m2:, :].copy()

    if scenario == "t3":
        X = rng.standard_t(df=3, size=(m, n)) / _np.sqrt(n)
        return X, X[:m2, :].copy(), X[m2:, :].copy()

    if scenario == "spiked":
        r = int(r_spike)
        V2 = _haar_frame(n, r, rng)
        V1_A = _haar_frame(m2, r, rng)
        V1_B = _haar_frame(m2, r, rng)
        E_A = rng.normal(scale=1.0 / _np.sqrt(n), size=(m2, n))
        E_B = rng.normal(scale=1.0 / _np.sqrt(n), size=(m2, n))
        X_A = V1_A @ V2.T + E_A
        X_B = V1_B @ V2.T + E_B
        X = _np.vstack([X_A, X_B])
        return X, X_A, X_B

    if scenario == "var":
        alpha = _np.array(var_alpha if var_alpha is not None else [0.5], dtype=float)
        nu = len(alpha)
        df = n + 2
        W = _wishart(df=df, n=n, rng=rng)
        Sigma = _np.linalg.inv(W)
        L = _np.linalg.cholesky(Sigma + 1e-10 * _np.eye(n))

        def simulate_segment(T: int) -> _np.ndarray:
            Y = _np.zeros((T + var_burnin + nu, n))
            Y[:nu, :] = rng.normal(scale=1.0 / _np.sqrt(n), size=(nu, n))
            for t in range(nu, Y.shape[0]):
                innov = L @ rng.normal(size=n)
                past = sum(alpha[k] * Y[t - (k + 1), :] for k in range(nu))
                Y[t, :] = past + innov
            return Y[var_burnin:, :][:T, :]

        X_A = simulate_segment(m2)
        X_B = simulate_segment(m2)
        X = _np.vstack([X_A, X_B])
        return X, X_A, X_B

    raise ValueError(f"Unknown scenario: {scenario!r}")

# ----- y generator under the shared DGP (only signal scale differs) -----
def generate_y_given_X(
    ...
    *,
    ...
    """
    Same DGP as generate_sim_data() except for the first component's scale.
    Returns: (y, true_idx, B)
    """
    m, n = X.shape
    if n < q_star:
        raise ValueError(f"n must be >= {q_star}, got {n}")
    rng = _np.random.default_rng(rng)

    half = n // 2
    b1 = _np.zeros(n); b1[:half] = float(b1_scale)
    B = _np.zeros((n, q_star)); B[:, 0] = b1
    for k in range(2, q_star + 1):
        B[k-1, k-1] = 1.0  # e_k

    def g(x): return (x - 2.0) ** 2
    def h(x): return _np.maximum(x, 0.0)
    ...
    idx_from_b1 = _np.arange(half, dtype=int)
    idx_from_pairs = _np.arange(q_star, dtype=int)
    true_idx = _np.unique(_np.concatenate([idx_from_b1, idx_from_pairs]))
    return y, true_idx, B

# ----- Generic split-aware training path using any model builder -----
def torch_nn_feature_selection_path_generic_from_splits(
    X_A: Array, X_B: Array, y_A: Array, y_B: Array,
    alpha: float, T: int,
    build_model,                        # Callable[[int], nn.Module]
    true_idx: Optional[Array] = None,
    batch_size: int = 128,
    ...
    assert 0.0 < alpha < 1.0 and T >= 1
    n = X_A.shape[1]; assert X_B.shape[1] == n
    # Global standardization on union
    X_full = _np.vstack([X_A, X_B])
    Xn_full, mu, sigma = standardize_global(X_full)
    m2 = X_A.shape[0]
    X1n = torch.tensor(Xn_full[:m2], dtype=torch.float32, device=device)
    X2n = torch.tensor(Xn_full[m2:], dtype=torch.float32, device=device)
    y1 = torch.tensor(y_A, dtype=torch.float32, device=device)
    y2 = torch.tensor(y_B, dtype=torch.float32, device=device)
    ...
    for t in range(1, T + 1):
        ...
        if (t % 10) == 0 or (t == T):
            ...
            sgn = _np.sign(xi1 * xi2)
            mag = psi_aggregate(_np.abs(xi1), _np.abs(xi2), method=psi_method)
            M = sgn * mag
            tau, Rp, Rm, FDPhat = strict_mirror_fdp_threshold(M, alpha=alpha)

            selected = _np.where(M > tau)[0] if _np.isfinite(tau) else _np.array([], dtype=int)
            FDR_true, TypeII = true_metrics(selected, true_idx, n)
            steps.append(t); taus.append(tau); Rps.append(Rp); Rms.append(Rm); FDPs.append(FDPhat)
            FDRs.append(FDR_true if FDR_true is not None else _np.nan)
            T2s.append(TypeII if TypeII is not None else _np.nan)

            xi1_list.append(xi1); xi2_list.append(xi2); M_list.append(M)
            last_xi1 = xi1; last_xi2 = xi2; last_M = M
            last_selected = selected; last_tau = float(tau)

    return PathResult(
        steps=_np.array(steps, dtype=int),
        tau=_np.array(taus, dtype=float),
        R_plus=_np.array(Rps, dtype=int),
        R_minus=_np.array(Rms, dtype=int),
        FDPhat=_np.array(FDPs, dtype=float),
        FDR_true=_np.array(FDRs, dtype=float) if (true_idx is not None and len(true_idx) > 0) else None,
        TypeII=_np.array(T2s, dtype=float) if (true_idx is not None and len(true_idx) > 0) else None,
        last_xi1=last_xi1, last_xi2=last_xi2, last_M=last_M,
        last_selected=last_selected, last_tau=last_tau,
        loss_steps=_np.array(loss_steps, dtype=int),
        loss_values=_np.array(loss_values, dtype=float),
        xi1_hist=_np.stack(xi1_list, axis=0) if xi1_list else _np.empty((0, n), dtype=float),
        xi2_hist=_np.stack(xi2_list, axis=0) if xi2_list else _np.empty((0, n), dtype=float),
        M_hist=_np.stack(M_list, axis=0) if M_list else _np.empty((0, n), dtype=float),
    )

# ----- Model-specific split-aware wrappers -----
def torch_nn_feature_selection_path_from_splits(
    X_A: Array, X_B: Array, y_A: Array, y_B: Array,
    alpha: float, T: int,
    true_idx: Optional[Array] = None,
    hidden_dims: List[int] = [64, 64],
    ...
    return torch_nn_feature_selection_path_generic_from_splits(
        X_A, X_B, y_A, y_B, alpha, T, build_model=_build, true_idx=true_idx,
        batch_size=batch_size, lr=lr, weight_decay=weight_decay, loss=loss,
        psi_method=psi_method, xi_batch=xi_batch, xi_mode=xi_mode,
        seed_model1=seed_model1, seed_model2=seed_model2
    )

def torch_cnn1d_feature_selection_path_from_splits(
    X_A, X_B, y_A, y_B, alpha, T,
    true_idx=None,
    # --- lift ---
    lift_dim: int | None = None,
    lift_init_mode: str = "he",
    ...
    batch_size=128, lr=5e-3, weight_decay=0.0,
    loss="mse", psi_method="min",
    xi_batch=2048, xi_mode="sumgrad",
    seed_model1: int = 11, seed_model2: int = 22,
):
    n = X_A.shape[1]
    q = n if lift_dim is None else int(lift_dim)
    def _build(n_in: int) -> nn.Module:
...
def torch_rnn_feature_selection_path_from_splits(
    X_A, X_B, y_A, y_B, alpha, T,
    true_idx=None,
    # --- lift ---
    lift_dim: int | None = None,
