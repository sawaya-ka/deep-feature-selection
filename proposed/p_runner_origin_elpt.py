...
def run_one_seed(seed:int, m:int, n:int, T:int, outdir:Path):
    ...
    Sigma = make_cov(n, kind="ar1", rho=0.75, blocks=None, rho_out=0.2,
             rank=None, spike=10.0, sigma2=1.0, lengthscale=10.0)
    X, y, true_idx, B = generate_data_elpt(m=m, n=n, rng=seed,Sig=Sigma)
    ...
    path = torch_nn_feature_selection_path(
        ...
        batch_size=256,
        lr=1e-3,                # constant LR (independent of X)
        weight_decay=1e-4,
        loss="mse",
        psi_method="mean",
        seed_split=2025,
    ...
    path_cnn = torch_cnn1d_feature_selection_path(
        ...
        strides=[1, 1, 1],
        fc_dims=[128, 64],
        activation="relu", init_mode="xavier",
        ...
        compute_every=10, xi_batch=1024,
