...
# あなたのユーティリティ（マルチクラス＋CE版）をインポート
from util_clsf import (
    generate_trig_softmax_data,
...
def run_one_seed(seed:int, m:int, n:int, T:int, outdir:Path, K:int=3):
    ...
    # マルチクラス人工データ（y は 0..K-1）
    X, y, true_idx, B = generate_trig_softmax_data(m=m, n=n, K=K, seed=seed)
    ...
    #     dim_feedforward=512, dropout=0.1,   # ← 516 ではなく 512 に調整
...
# ========== ランチャ（並列実行） ==========
def launch(seeds, m, n, T, outdir, n_gpu=None, K:int=3):
    t0 = time.time()
    try:
        import torch
        if n_gpu is None:
    ...
    try:
        ...
        for i, s in enumerate(seeds):
            ...
            cmd=[sys.executable,__file__,"--mode","worker","--seed",str(s),
                 "--m",str(m),"--n",str(n),"--T",str(T),"--outdir",str(outdir),"--K",str(K)]
...
def main():
    ...
    if args.mode=="worker":
        run_one_seed(args.seed,args.m,args.n,args.T,Path(args.outdir),K=args.K); return
    if args.mode=="merge":
    ...
    launch(seeds,args.m,args.n,args.T,args.outdir,args.ngpu,K=args.K)
