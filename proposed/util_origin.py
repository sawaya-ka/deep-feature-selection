...
## DGP for elliptical design

from numpy.linalg import eigh, cholesky

def make_cov(n, kind="ar1", rho=0.5, blocks=None, rho_out=0.2,
             rank=None, spike=10.0, sigma2=1.0, lengthscale=10.0):
    """
    Return Sigma (n x n) as float64.
    kind:
      - "ar1":      Sigma_{ij} = rho^{|i-j|}
      - "equicorr": (1-rho)I + rho * 11^T
      - "block":    block equicorrelation (blocks: list of sizes)
      - "toeplitz-exp": exp(-|i-j|/lengthscale)
      - "spiked":   LL^T + sigma2*I with rank 'rank' and spike strength
    """
    ...
    if kind == "ar1":
        ...
        Sigma = (1.0 - rho) * I + rho * np.ones((n, n), dtype=np.float64)

    elif kind == "block":
        assert blocks is not None and sum(blocks) == n
        Sigma = np.full((n, n), rho_out, dtype=np.float64)
        start = 0
        for b in blocks:
            sl = slice(start, start+b)
            Sigma[sl, sl] = (1.0 - rho) * np.eye(b) + rho * np.ones((b, b))
            start += b

    elif kind == "toeplitz-exp":
        idx = np.arange(n)
        d = np.abs(idx[:,None]-idx[None,:])
        Sigma = np.exp(-d / float(lengthscale))

    elif kind == "spiked":
        assert rank is not None and rank > 0
        # ランダム直交方向に spike 固有値を置く
        G = np.random.randn(n, rank)
        U, _ = np.linalg.qr(G)                 # n x rank （直交）
        eigs = spike * np.ones(rank)
        L = U * np.sqrt(eigs)[None, :]        # n x rank
        Sigma = L @ L.T + sigma2 * I
    else:
...
def generate_data_elpt(m: int, n: int, rng=None, Sig=None):
    """
    Generate (X, y, true_idx, B) with
        y = g(b1^T x) + sum_{k=2}^{q*} h(b_k^T x) * (b_{k-1}^T x) + eps,
        eps ~ N(0,1), x ~ N(0, I_n),
    ...
    # X ~ N(0, Sigma)
    X = st.multivariate_normal.rvs(cov=Sig,size=(m),random_state=int(1)) #
