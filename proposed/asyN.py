...
mn_list = [[200000, 500], [2000, 500], [20, 500]] # (m,n): effective m is the half
...
for i in range(3):
    ...
    k = int(n/2)
    true_idx = np.sort(rng.choice(n, size=k, replace=False))
    w_true = np.zeros(n)
    # w_true[true_idx] = rng.normal(scale=2.0, size=k)
    w_true[true_idx] = np.append(np.ones(int(k/2)), -np.ones(int(k/2)))
    w_true = w_true / np.linalg.norm(w_true) * 5
    y = sinx(X @ w_true)  + rng.normal(scale=0.1, size=m)  # regression

    PBperp = np.eye(n) - np.matrix(w_true).T@np.matrix(w_true)/(w_true@w_true)
    PBperp = np.array(PBperp)
