# ============================================================
# Unified code: CNN/LSTM = old behavior, MLP/Transformer = new
# ============================================================
import os, math
import numpy as np
import torch
...
def make_lr_schedule(T: int, base_lr: float = 1e-3,
                     warmup_ratio: float = 0.1, min_lr_ratio: float = 1.0):
...
# (NEW) orientation for v2 (not used by CNN/LSTM)
@torch.no_grad()
...
# 共通：sum of input-gradients on standardized X
...
# ============================================================
# (NEW) Transformer …… こっちら（局所化トークン + ID + ゲート）
# ============================================================
...
class TorchTransformer1D_Lifted(nn.Module):
    ...
    def __init__(self,
        ...
        if self.pos_mode == "sin":
            # 既に定義済みの SinusoidalPositionalEncoding を使う前提
    ...
    def _embed(self, s: torch.Tensor) -> torch.Tensor:
        if self.id_encoding == "linear":
            # s: (B,L,1) or (B,L,p); token_W: (L,d_model,d_tok)
...
def torch_transformer_feature_selection_path(
    X, y, alpha, T, true_idx=None,
    lift_type="identity", patch_size=8, lift_dim=None,
    id_encoding="linear",
    ...
    lr_schedule=None,
    lift_init_mode="he", lift_fixed_sigma=0.01, lift_bias=True,   # ← 追加
):
    n = X.shape[1]
    q = n if (lift_type in ("identity","patch") or lift_dim is None) else int(lift_dim)
    def _build(n_in: int) -> nn.Module:
        return TorchTransformer1D_Lifted(
            input_dim=n_in, lift_dim=(q if lift_type=="gaussian" or lift_type=="dense" else n_in),
            ...
            fc_dims=fc_dims,
            lift_init_mode=lift_init_mode, lift_fixed_sigma=lift_fixed_sigma, lift_bias=lift_bias
        )
    if lr_schedule is None:
